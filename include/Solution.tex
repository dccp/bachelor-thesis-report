...
\node[rectangle,draw] (backend) [below=1cm of frontend] {Backend};
\node[rectangle,draw] (xfer) [below=1cm of backend] {Docker transfer};
\node[rectangle,draw] (docker) [below=1cm of xfer] {Docker daemon};
\node[rectangle,draw] (web3) [left=1cm of backend] {web3.js};
\node[rectangle,draw] (ethcl) [below=1cm of web3] {Ethereum client};
\node[cylinder, shape border rotate=90, draw] (ethblk) [below=1cm of ethcl] {Ethereum blockchain};
...
\path[-latex] 
    ...
    (web3)      edge    node    {JSON-RPC}          (ethcl)
    (ethcl)     edge    node    {DEVP2P}            (ethblk);
\end{tikzpicture}
