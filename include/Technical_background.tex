...
A new block is created approximately every 10 minutes, with all transactions that occurred since the last block creation bulked in it. Figure~\ref{blockchain} shows the chain of blocks. Each block contains its parent hash; a nonce that proves the validity of the block; and all the transactions in the block. To calculate the current state of the network, the whole graph of blocks must be traversed.
